    ...
    function() {
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          var popover_container = element[0].querySelector(
          ...
          triangle_height = Math.sqrt(
            (triangle_div_side * triangle_div_side) / 2
          );
          triangle_diagonal = Math.sqrt(
            triangle_div_side * triangle_div_side * 2
          );
          var mode = attrs.mode || "click";
          var closeOnClick =
            attrs.closeOnClick === undefined
              ? mode == "click"
                ? true
                : false
              : attrs.closeOnClick === "true";

          var closeOnMouseleave =
            attrs.closeOnMouseleave === undefined
              ? mode == "mouseover"
                ? true
                : false
          ...
          element[0].addEventListener(mode, function() {
            ...
            if (mode == "click" && closeOnClick) {
              ...
              buffer = document.querySelector(".openFlag");
              if (buffer) {
                buffer.classList.remove("openFlag");
                buffer.classList.add("hide-popover-element");
              }
              buffer = document.querySelector(".openFlagTriangolo");
              if (buffer) {
                buffer.classList.remove("openFlagTriangolo");
                buffer.classList.add("hide-popover-element");
              }

              popover.classList.add("openFlag");
              triangle.classList.add("openFlagTriangolo");

              popover.classList.toggle("open-hide-popover-element");
              ...
              popover_container.classList.toggle(
                "popover-floating-animation-" + attrs.direction
              );
              ...
              popover_container.classList.add(
                "popover-floating-animation-" + attrs.direction
              );
            }
            ...
            if (attrs.template) {
              var templateElement = element[0].querySelector(
            ...
            switch (attrs.direction) {
              ...
              case "bottom":
                ...
                popover.style.left = (parent_width - popover_width) / 2 + "px";
                triangle.style.top =
                  -(triangle_rect_div_side - triangle_height) + "px";
                triangle.style.left =
                  (parent_width - triangle_rect_div_side) / 2 + "px";
                break;
              ...
              case "right":
                popover.style.top =
                  (parent_height - popover_height) / 2 - parent_height + "px";
                popover.style.left = parent_width + triangle_height + "px";
                triangle.style.top =
                  (parent_height - triangle_rect_div_side) / 2 -
                  parent_height +
                  "px";
                triangle.style.left =
                  parent_width -
                  (triangle_rect_div_side - triangle_height) +
                  "px";
                break;
